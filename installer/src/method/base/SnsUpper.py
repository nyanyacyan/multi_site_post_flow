...
class OverrideApiRequest(ApiRequest):
    ...
    def apiRequest(
        ...
        return super().apiRequest(method, endpointUrl, headers, params, json, maxRetry, delay, notifyFunc)
...
class XPoster:
    def __init__(self, xConsumerKey: str, xConsumerSecret: str, xAccessToken: str, xAccessTokenSecret: str, debugMode=True):
        ...
        self.auth = (self.consumer_key, self.consumer_secret, self.access_token, self.access_token_secret)
...
# 非同期処理
    ...
    async def process(self, imageUrl: str, message: str, notifyFunc: Optional[Callable[[str], None]] = None):
        ...
        if upLoadResponse.status_code == 200:
            ...
            if XResponse.status_code == 201:
                ...
                self.logger.error(f"X 送信失敗: {XResponse.status_code},{XResponse.text}")

        else:
            self.logger.error(f"X の写真の送信失敗: {XResponse.status_code},{XResponse.text}")
...
class InstagramPoster:
    def __init__(self, instAccessToken: str, instBusinessAccountId: str, debugMode=True):
        # logger
        self.getLogger = Logger(__name__, debugMode=debugMode)
        self.logger = self.getLogger.getLogger()
...
#! 画像は公開されてる写真ではないとNG
    ...
    async def _getMediaId(self, imageUrl: str, notifyFunc: Optional[Callable[[str], None]] = None):
        ...
        try:
            ...
            if response:
                if response.status_code == 200:
                    ...
                    self.logger.debug(f"resがエラーで帰ってきてる{response.status_code}")
