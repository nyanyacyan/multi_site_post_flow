...
class ThreadEventNoUpdate(QObject):
    ...
    def _date_end_time_task(self, stop_event: threading.Event, label: QLabel, process_func: Callable, user_info: Dict, gss_info: str, interval_info: Dict):

        # メイン処理の再実行
        self.loop_process.main_task(stop_event=stop_event, label=label, process_func=process_func, user_info=user_info, gss_info=gss_info, interval_info=interval_info)
