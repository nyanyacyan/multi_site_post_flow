...
# 自作モジュール
from installer.src.method.base.notify import LineNotify, ChatworkNotify, SlackNotify, DiscordNotify
...
# @patch('method')→この中にいれるmethodがモック化される→モック化されたものは引数に渡す必要がある
    ...
    def testLineApiRequestSuccess(self):
        ...
        # インスタンスの作成
        instance = LineNotify(debugMode=True)
        ...
        with patch('builtins.open', read_data=mock_file_data) as mock_file:
            with patch('requests.post', return_value=resultJson) as mockRequest:
                ...
                # メソッドの呼び出し
                result = instance.line_image_notify(lineToken=lineToken, message=message, image_path=image_path)
