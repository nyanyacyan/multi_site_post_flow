...
# 自作モジュール
from installer.src.method.base.spreadsheetRead import GetDataGSSAPI
...
# ----------------------------------------------------------------------------------
    ...
    def testGetDataFrameFromGss(self):
        ...
        with patch.object(GetDataGSSAPI, 'client', new_callable=PropertyMock) as mockClientProp:
            # モック化→空の箱にいれる
            ...
            # インスタンスの作成
            instance = GetDataGSSAPI(debugMode=True)

            # メソッドの呼び出し
            instance.getDataFrameFromGss(KeyName=KeyName, spreadsheetId=spreadsheetId, sheetName=sheetName)
