...
# 自作モジュール
from installer.src.method.base.ApiRequest import ApiRequest
...
# @patch('method')→この中にいれるmethodがモック化される→モック化されたものは引数に渡す必要がある
    ...
    async def testApiRequestSuccess(self):
        ...
        # インスタンスの作成
        instance = ApiRequest(debugMode=True)

        # importしたクラスの中から’client’属性を抽出してモックに置き換える
        with patch.object(instance, 'apiRequest', return_value=resultJson) as mockRequest:

            # メソッドの呼び出し
            result = await instance.apiRequest(method=method, endpointUrl=endpointUrl, headers=headers, params=params, json=json)
