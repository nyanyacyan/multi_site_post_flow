...
# 自作モジュール
from installer.src.method.base.AiOrder import ChatGPTOrder
...
# @patch('method')→この中にいれるmethodがモック化される→モック化されたものは引数に渡す必要がある
    ...
    async def testAiOrderSuccess(self):
        ...
        # インスタンスの作成
        instance = ChatGPTOrder(debugMode=True)

        # importしたクラスの中から’client’属性を抽出してモックに置き換える
        with patch.object(instance, 'chatGptRequest', AsyncMock(return_value=resultJson)) as mockRequest:

            # メソッドの呼び出し
            await instance.resultSave(prompt=prompt, snsKinds=snsKinds, fixedPrompt=fixedPrompt, model=model, apiKey=apiKey, maxTokens=maxTokens, maxlen=maxlen)
    ...
    async def testApiRequestError(self):
        ...
        with patch('aiohttp.ClientSession.request', side_effect=aiohttp.ClientError('ClientError')):
            with pytest.raises(SystemExit) as exc_info:
                ...
                # メソッドの呼び出し
                await instance.apiRequest(method=method, endpointUrl=endpointUrl, headers=headers, params=params, json=json)
